...
- Schema lives in `supabase/schema.sql` and is applied manually via the Supabase SQL Editor. When changing tables/functions, update the file and re-run it in the dashboard. The app calls two Postgres functions via RPC: `get_or_create_item(text)` and `cook_meal(uuid)` (`src/lib/api.ts`).
...
- No router — tab switching is plain `useState` in `src/App.tsx`. New views go in `src/components/` and are added to `TABS`.
